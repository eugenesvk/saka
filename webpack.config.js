...
module.exports = function webpackConfig(env, argv) {
      ...
      rules: [
        ...
        { test:/\.s[ac]ss$/i, use:[require.resolve('style-loader'),require.resolve('css-loader'), {loader:require.resolve('sass-loader'),options:{sassOptions:{
            importer(url, prev) {
              ...
              return { file:url }; }}}} ]},
        { test:/\.md$/      , use:[require.resolve('html-loader'),
          {loader:require.resolve('markdown-loader'),options:{renderer}}]}
      ]
    },
    plugins: [
      ...
      new GenerateJsonPlugin( 'manifest.json',
        merge( require('./manifest/common.json'),
  ...
  // 1. SAKA_DEBUG: boolean 'true' for development builds; 'false' for production build
  //   If you want something to run only in development, use
  //     if (SAKA_DEBUG) { console.log(variable); }.
  //   All code within will be removed at build time in production builds.
  // platform controls:
  // 1. SAKA_PLATFORM: string('chrome' | 'firefox' | 'edge') provides platform specific features, e.g. use shadow DOM on chrome but css selectors on firefox and edge for link hint styling
